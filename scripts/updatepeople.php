 ...
 */
   try {
            ...
            $query = $dbh->prepare("INSERT INTO people (id, name, birthdate, deathdate, overview, birth_place, imdb_url, tmdb_url, movie_count, series_count) VALUES (?, ?, STR_TO_DATE(?,'%Y-%m-%dT%H:%i:%s'), STR_TO_DATE(?,'%Y-%m-%dT%H:%i:%s'), ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE id=id");
